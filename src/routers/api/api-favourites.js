...
router.get('/api/favourites', auth, async (req, res) => {
    ...
    try {
        ...
        let movieInfo = await getMovies(favourite)

        
        if (req.query.email == 'true') {
            await mail.sendFavouritesMail({username: req.user.username, email: req.user.email}, movieInfo)
        }

        return res.status(201).send(movieInfo);
